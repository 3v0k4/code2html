(() => {
  ...
  var setupCopyHtml = () => {
    const inline = (node) => {
      [...document.styleSheets].filter((sheet) => sheet.media.mediaText !== "not all").flatMap((sheet) => [...sheet.cssRules]).filter((rule) => {
        let isMatch = false;
        try {
      ...
      }).flatMap((rule) => [...rule.style].map((property) => [property, rule.style[property]])).forEach(([property, value]) => node.style[property] = value);
    };
    const inlineRecursive = (node) => {
      if (node.classList.contains("cm-gutters")) {
        return;
      }
    ...
    copyButton.addEventListener("click", async (event) => {
      ...
      setInterval(() => copyButton.innerText = buttonLabel, 3e3);
    });
  };
  var setupVisibleEditor = () => {
  ...
  var setupInvisibleEditor = () => {
    const unstyleRecursive = (node) => {
      if (node.classList.contains("styled")) {
        return;
      }
      if (node.children.length === 0) {
        node.style = "";
    ...
    if (window.invisibleEditor) {
      window.invisibleEditor.destroy();
    }
